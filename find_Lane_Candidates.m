...
function [ msg ] = find_Lane_Candidates(Likelihoods, Templates)
   ...
   %%In center coordinate System
    ...
    Lane_Int_Base_tmp     = zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1);
    
    Lane_Int_Purview_tmp  = zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1);
    
    Lane_Int_Weights_tmp  = zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1);
    ...
    for row_current = 0: (size(Focussed_TOT_P,1)-1)
        ...
        for col_current = 0: (size(Focussed_TOT_P,2)-1)
            ...
            if(prob>0)
                ...
                Lane_Int_Base_tmp   (row_current*size(Focussed_TOT_P,2) + (col_current +1))    =  ( double(bottom- y) *2^7  )./single(tanGradient)  +  single(x);
                Lane_Int_Purview_tmp(row_current*size(Focussed_TOT_P,2) + (col_current +1))    =  ( double(horizon-y) *2^7  )./single(tanGradient)  +  single(x);
                
                Lane_Int_Weights_tmp(row_current*size(Focussed_TOT_P,2) + (col_current +1))          =  single(prob)*depthSqr* 2^-7;
     ...
     Lane_Int_Base      =   single(zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1)  );
     Lane_Int_Purview   =   single(zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1)  );
     Lane_Int_Weights   =   single(  zeros(size(Focussed_TOT_P,1)*size(Focussed_TOT_P,2),1));
   
    
    
     Index =1;     
    for i= 1: size(Lane_Int_Base_tmp)

        if (Lane_Int_Weights_tmp(i) > 0)

            if ( Lane_Int_Base_tmp(i) > BASE_HISTOGRAM_BINS(1)-BASE_HISTOGRAM_STEP/2    &&   Lane_Int_Base_tmp(i)    < BASE_HISTOGRAM_BINS(end)+ BASE_HISTOGRAM_STEP/2)


              if (  Lane_Int_Purview_tmp(i) > -VP_RANGE_H-HORIZON_HISTOGRAM_STEP/2        &&   Lane_Int_Purview_tmp(i) < VP_RANGE_H+HORIZON_HISTOGRAM_STEP/2  )                

                                Lane_Int_Base(Index)      = Lane_Int_Base_tmp(i);                        
                                Lane_Int_Purview(Index)   = Lane_Int_Purview_tmp(i);
                                Lane_Int_Weights(Index)   = Lane_Int_Weights_tmp(i);
